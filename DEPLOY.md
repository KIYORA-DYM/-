...
## 4. Googleログインを有効にする(任意)

1. https://console.cloud.google.com/ でプロジェクトを作成
2. 「APIとサービス」→「OAuth同意画面」を設定
3. 「認証情報」→「認証情報を作成」→「OAuthクライアントID」(種類: ウェブアプリケーション)
   - 承認済みのJavaScript生成元に本番URL(`https://task-manager-hu9u.onrender.com`)を追加
4. 発行された クライアントID(`〇〇.apps.googleusercontent.com`)を、Renderの環境変数に追加

   | Key | Value |
   |---|---|
   | `GOOGLE_CLIENT_ID` | 発行されたクライアントID |
   | `VITE_GOOGLE_CLIENT_ID` | 同じクライアントID(ビルド時にフロント側で使用) |

5. 環境変数を追加したら、Renderで「Manual Deploy」→「Deploy latest commit」を実行して再ビルド

## 新規ユーザーの承認について

- 最初に登録したアカウント(二谷様)が自動的に管理者になっています。
- それ以降に登録(メール/パスワードまたはGoogle)した人は「承認待ち」状態になり、ログインできません。
