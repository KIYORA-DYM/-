...
// GET /api/actions?completed=0 — flat list of next actions across all deals
...
router.patch("/:id", requireAuth, async (req, res) => {
  const existing = (await db.execute({ sql: "SELECT * FROM actions WHERE id = ?", args: [req.params.id] })).rows[0];
  if (!existing) {
...
router.delete("/:id", requireAuth, async (req, res) => {
  const result = await db.execute({ sql: "DELETE FROM actions WHERE id = ?", args: [req.params.id] });
